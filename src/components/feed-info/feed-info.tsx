...
export const FeedInfo = () => {
  ...
  useMemo(() => {
    orders?.forEach(item => item.status === 'done' ? done.push(item.number) : preparing.push(item.number));
  }, [orders]) 
                ...
                <div className={styles.info_orders}>
                    <div className={styles.done_prep}>
                        ...
                        <div className={styles.column}>
                            {done.slice(0, maxOrdersShown).map(item => (
                                <p className="text text_type_digits-default mb-2" style={{color: '#00CCCC'}}>{item}</p>    
                            ))}
                        </div>
                    </div>
                    <div className={styles.done_prep}>
                        ...
                        <div className={styles.column}>
                            {preparing.slice(0, maxOrdersShown).map(item => (
